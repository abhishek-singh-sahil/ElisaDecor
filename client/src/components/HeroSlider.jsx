...
export default function HeroSlider({ slides, settings, globalSettings, products }) {
  ...
  return (
    <section className="relative w-full h-[90vh] md:h-[95vh] bg-primary-dark overflow-hidden flex items-center">
      ...
      {slides.map((slide, idx) => (
        ...
        >
          <img
            ...
            alt={slide.title}
            className="w-full h-full object-cover object-center scale-102 transition-transform duration-[10000ms]"
          />
          <div
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 md:px-8 w-full text-white space-y-6 select-none">
        <div className="max-w-3xl space-y-4 md:space-y-6">
          {currentSlide.eyebrow && (
            <span className="inline-block text-xs md:text-sm font-bold tracking-widest text-brass uppercase animate-none">
              {currentSlide.eyebrow}
            </span>
          )}
          
          <h1 className="text-4xl sm:text-5xl md:text-7xl font-bold font-serif leading-tight tracking-tight text-white select-text">
            {currentSlide.title}
          </h1>

          {currentSlide.subtitle && (
            <p className="text-xl md:text-2xl font-light font-serif italic text-zinc-200">
              {currentSlide.subtitle}
            </p>
          )}

          {currentSlide.description && (
            <p className="text-sm md:text-base text-zinc-300 max-w-xl font-normal leading-relaxed select-text">
              {currentSlide.description}
            </p>
          )}
        </div>
        ...
        <div className="flex flex-wrap gap-4 pt-2">
          {currentSlide.ctaText1 && (
            <Link
              to={currentSlide.ctaUrl1 || '/products'}
          ...
          >
            <Mail size={14} />
            {currentSlide.ctaText2 || 'Request Quote'}
          </button>
        </div>
      </div>
